...
pub fn find_app_by_name_fast(
    ...
    for dir in &dirs {
            ...
            })
        {
            ...
            // Try cache first
            let app_result: Result<desktop::App, eyre::Report> = if let Ok(Some(cached_app)) = desktop_cache.get(file_path) {
                Ok(cached_app)
            } else {
                // Parse the file
...
pub fn launch_program_directly(cli: &cli::Opts, program_name: &str) -> Result<()> {
    ...
    let apps_receiver = desktop::read_with_options(dirs, &db, cli.filter_desktop, cli.list_executables_in_path);
